...
from modules.linux_utils.linux_utils import init_ssh_client, backup_existing_images, replace_step_id_in_filenames
from modules.media_utils.video_ops import convert_webm_to_mp4
from modules.qc_exceptions import StepModification
...
import math
from scripts.archive.feishu_bitable.feishu_bitable import get_record_by_id
from scripts.archive.feishu_bitable.update_sub_instruction import extract_ids, extract_id_flow_dict, input_flows
from scripts.regenerate_screenshots import regenerate_screenshots_by_step
    ...
    for end_image_record in end_image_records:
        ...
        for flow in flows:
            if flow.id == instruction_id:
                ...
                #
                # remote_img_dirs = ["/var/www/html/storage/frames_raw", "/var/www/html/storage/frames_marked"]
...
def jul8_find_records_with_redo():
    ...
    for redo_record in redo_records:
        ...
        while True:
            ...
            redo_record = get_record_by_id(app_token, table_id, [parent_record_id])
            # print(parent_record)
...
def main():
    ...
    for dir in [Path(r"C:\Users\dehan\Desktop\WedAgentPipeline_hl\jsons\final_dash_jsons")]:
        for json_file in dir.rglob("nytimes_2025-07-08_part_1.json"):
            ...
            with open(OUTPUT_DIR / f"{dir.name}.json", 'w', encoding='utf-8') as f:
                f.write(json.dumps(json_flow, ensure_ascii=False, indent=4))

    img_files = IMG_OUTPUT_DIR.rglob("*.jpeg")
    upload_images_to_server(img_files)
...
def jul10_candidate_list():
    ...
    # for dir in [Path(r"C:\Users\dehan\Desktop\WedAgentPipeline_hl\jsons\final_dash_jsons")]:
    for dir in [Path(r"C:\Users\dehan\Desktop\a666_redo_flows_batch_20250704_182929")]:
        for json_file in dir.rglob("*.json"):
            logger.info(f"Processing {json_file.name}")
            ...
            for flow in flows:
                for idx, step in enumerate(flow.steps):
                    videos = video_dir.rglob(f"{step.recording_id}.webm")
                    video_path = next(videos, None)
                    if not video_path:
                        continue

                    # video_path = list(videos)[0]
                    if not video_path.exists():
                        raise StepModification(
                            f"Step {step.id} missing video file: {step.recording_id} under {video_dir}")
                    ...
                    regenerate_screenshots_by_step(step, mp4_path)
...
if __name__ == "__main__":
    ...
    main()  # flows跑mp4/webm截帧
    # jul10_candidate_list()  # flows跑candidate方法