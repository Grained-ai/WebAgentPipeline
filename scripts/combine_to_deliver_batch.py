...
INPUT_CSV = Path(
    '/Users/anthonyf/projects/grainedAI/WebAgentPipeline/src/batches/20250522/delivery/QC_WebAgent_任务总表_交付表 (3).csv')
SERVER_IMG_BASE = 'http://3.145.59.104/checkingimg'


def group_to_deliver_instructions(csv_path: Path):
    ...
    with csv_path.open(newline="", encoding="utf-8-sig") as f:
        ...
        for row in reader:
            ...
            if 'w_check' in batch_id:
                b += "_w_check"
            json_name_batch_map[b.split('.json')[0]] = batch_id
            ...
            if entries:
                for entry in entries:
                    ...
                    for step in entry['steps']:
                        if 'img' in step:
                            step['imgSave'] = SERVER_IMG_BASE + "/" + str(entry_from_json.stem) + '/' + step[
                                'id'] + "_marked.jpeg"
    ...
    for item in matched:
        logger.info(item)
        batch_id = json_name_batch_map.get(item['from_json'].split('.json')[0], 'UNKNOWN')
        if batch_id == 'UNKNOWN':
            logger.error(json.dumps(item, indent=4,ensure_ascii=False))
        if batch_id not in out:
            out[batch_id] = []
        out[batch_id].append(item)
    for batch_id in out:
    # 4) 写出结果
        now = datetime.now()
        out_file = out_dir / f"{batch_id}_{now.strftime("%Y%m%d_%H%M%S")}.json"
        out_file.write_text(
            json.dumps(out[batch_id], ensure_ascii=False, indent=4),
            encoding="utf-8"
        )
        logger.success(f"✔️ Batch owner_check 共写入 {len(out[batch_id])} 条记录 到 {out_file}")

if __name__ == "__main__":
    group_to_deliver_instructions(INPUT_CSV)