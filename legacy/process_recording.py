...
def check_and_add_end_step(json_file, data):
...
def main(json_file):
    ...
    # 检查并添加end步骤
    data = check_and_add_end_step(json_file, data)
...
if __name__ == "__main__":
    ...
    batch_dir = Path("/Users/anthonyf/projects/grainedAI/WebAgentPipeline/src") # Put the parent dir of all jobs

    todo_jobs = glob.glob(str(batch_dir/"*_*"))

    job_status = {}

    for job in tqdm(todo_jobs):
        logger.info(f"Working on: {job}")
        job_folder = Path(job)
        # if "_1" not in job_folder.name:
        #     logger.error(f"{job_folder} is not of form xxx_1. Please check.")
        #     job_status[str(job_folder)] = {'status': False, 'log': f"{job_folder} is not of form xxx_1. Please check."}
        #     continue
        job_name = job_folder.name.split('_')[0] # xxx_xxx -> xxx
        job_raw_image_dir = job_folder.parent/str(job_name+"_raw")
        if job_raw_image_dir.exists():
            logger.success(f'{job_name} already finished.')
            job_status[str(job_folder)] = {'status': True}

            continue
        job_json = job_folder/f'{job_name}.json'
        try:
            main(str(job_json))
            job_status[str(job_folder)] = {'status': True}
        except Exception as e:
            logger.error(f"Failed to run {json}: {str(e)}")
            logger.warning(traceback.print_exc())
            job_status[str(job_folder)] = {'status': False, 'log': str(traceback.print_exc())}
